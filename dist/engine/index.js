    ...
    if (Object.keys(filterObj).length > 0) {
        for (const record of filtered) {
            if (record.urgency === undefined) {
                record.urgency = tasksByUuid.get(record._id)?.urgency ?? computeUrgency(record, tasksByUuid, blockingIndex);
            }
        }
    }
...
function validatePreInsert(description, attrs) {
