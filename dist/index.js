...
function createServer(config) {
    ...
    server.registerTool("task_list", {
        title: "List Tasks",
        description: "Query and return tasks matching a filter expression. Returns a JSON array of task objects with all fields (uuid, id, description, status, priority, due, tags, urgency, etc). " +
        ...
        inputSchema: z.object({
            filter: z.string().describe("Filter expression to match tasks. Examples: 'status:pending', 'project:backend +bug', " +
    ...
    server.registerTool("task_modify", {
        title: "Modify Task",
        description: "Update one or more tasks matching a filter. Only provided fields are changed — omitted fields are left untouched (partial update). " +
    ...
    server.registerTool("task_done", {
        title: "Complete Task",
        description: "Mark a task as completed, setting its status to 'completed' and recording the end timestamp. " +
    ...
    server.registerTool("task_delete", {
        title: "Delete Task",
        description: "Soft-delete a task by setting its status to 'deleted'. The task remains in the database and can be restored with task_undo. " +
    ...
    server.registerTool("task_annotate", {
        title: "Annotate Task",
        description: "Add a timestamped text note to a task. Annotations are short, append-only notes for tracking progress, decisions, or handoff context across sessions. " +
    ...
    server.registerTool("task_start", {
        title: "Start Task",
        description: "Mark a task as actively being worked on by recording a start timestamp. " +
    ...
    server.registerTool("task_undo", {
        title: "Undo",
        description: "Undo the most recent single operation (add, modify, delete, done, start, stop, annotate). " +
    ...
    server.registerTool("task_info", {
        title: "Task Info",
        description: "Get the full JSON details for a single task by ID or UUID. Returns all fields including annotations, dependencies, docs, and computed urgency. " +
        ...
        if (tasks.length === 0) {
            return {
                content: [{ type: "text", text: "No task found with that ID." }],
                isError: true,
            };
        }
    ...
    server.registerTool("task_projects", {
        title: "List Projects",
        description: "Return a JSON array of all project names that have at least one pending or recurring task. " +
    ...
    server.registerTool("task_tags", {
        title: "List Tags",
        description: "Return a JSON array of all tags that have at least one pending or recurring task. " +
    ...
    server.registerTool("task_denotate", {
        ...
        description: "Remove a specific annotation from a task by exact text match. The annotation text must match exactly (case-sensitive). " +
    ...
    server.registerTool("task_purge", {
        title: "Purge Task",
        description: "Permanently and irreversibly remove a deleted task from the database. " +
    ...
    server.registerTool("task_import", {
        title: "Import Tasks",
        description: "Bulk-create tasks from a JSON array. Each object must have a 'description' field; all other fields (project, tags, priority, due, status, depends, recur) are optional. " +
        ...
        inputSchema: z.object({
            tasks: z.string().describe("JSON array of task objects. Required field: 'description'. Optional: 'project', 'tags' (string[]), 'priority' (H/M/L), " +
    ...
    server.registerTool("task_count", {
        title: "Count Tasks",
        description: "Return the count of tasks matching a filter as a plain number. More efficient than task_list when you only need the count, not the task data. " +
            "Uses the same filter syntax as task_list.",
        inputSchema: z.object({
            filter: z.string().describe("Filter expression. Same syntax as task_list. Examples: 'status:pending', '+OVERDUE', 'project:backend +bug'. " +
    ...
    server.registerTool("task_log", {
        ...
        description: "Record a task that is already completed, creating it directly in 'completed' status with an end timestamp. " +
    ...
    server.registerTool("task_duplicate", {
        ...
        description: "Create a new pending task by copying an existing one, optionally overriding specific fields. " +
    ...
    server.registerTool("task_doc_write", {
        ...
        description: "Attach or replace a markdown document on a task (spec, design, context, handoff notes). " +
    ...
    server.registerTool("task_doc_read", {
        title: "Read Task Doc",
        description: "Read the markdown document attached to a task. Returns the document content as text, or an error if no document is attached. " +
        ...
        if (doc === null) {
            return {
                content: [{ type: "text", text: "No doc attached to this task." }],
                isError: true,
            };
        }
    ...
    server.registerTool("task_doc_delete", {
        ...
        description: "Remove the markdown document attached to a task. Clears the +doc tag and has_doc field. " +
    ...
    server.registerTool("task_archive", {
        title: "Archive Old Tasks",
        description: "Move completed and deleted tasks older than N days to a quarterly archive segment, keeping the active set small and fast. " +
    ...
    server.registerTool("task_archive_list", {
        title: "List Archive Segments",
        description: "Return a JSON array of available archive segment paths (e.g. 'archive/archive-2026-Q1.json'). " +
    ...
    server.registerTool("task_archive_load", {
        title: "Load Archived Tasks",
        description: "Load and return tasks from an archive segment as a read-only JSON array for inspection. " +
