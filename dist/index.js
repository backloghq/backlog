...
function createServer(config) {
    ...
    server.registerTool("task_annotate", {
        title: "Annotate Task",
        description: "Add a timestamped text note to a task. Returns a confirmation on success. Errors if the task is not found. " +
    ...
    server.registerTool("task_info", {
        title: "Task Info",
        description: "Get the full JSON object for a single task by ID or UUID. Returns all fields including annotations, dependencies, has_doc, and computed urgency score. " +
    ...
    server.registerTool("task_projects", {
        title: "List Projects",
        description: "Return a JSON array of all project names that have at least one pending or recurring task. Returns an empty array if no projects exist. " +
    ...
    server.registerTool("task_tags", {
        title: "List Tags",
        description: "Return a JSON array of all tags that have at least one pending or recurring task. Returns an empty array if no tags exist. " +
    ...
    server.registerTool("task_count", {
        title: "Count Tasks",
        description: "Return the count of tasks matching a filter as a plain number (e.g. '5'). Returns '0' if no tasks match. " +
    ...
    server.registerTool("task_log", {
        ...
        description: "Record a task that is already completed, creating it directly in 'completed' status with an end timestamp. Returns 'Task logged.' on success. " +
    ...
    server.registerTool("task_doc_write", {
        ...
        description: "Attach or replace a markdown document on a task (spec, design, context, handoff notes). Returns a confirmation on success. " +
    ...
    server.registerTool("task_doc_read", {
        title: "Read Task Doc",
        description: "Read the markdown document attached to a task. Returns the full document content as text. " +
    ...
    server.registerTool("task_archive", {
        ...
        description: "Move completed and deleted tasks older than N days to a quarterly archive segment (cold storage), keeping the active set small and fast. " +
    ...
    server.registerTool("task_archive_list", {
        ...
        description: "Return a JSON array of available archive segment paths (e.g. 'archive/archive-2026-Q1.json'). Returns an empty array if no archives exist. " +
