...
export function resolveDate(input: string): Date {
  ...
  switch (lower) {
    ...
    // Week boundaries
    case "sow": case "soww": return startOfWeek(now);
    case "eow": case "eoww": return endOfWeek(now);
  ...
  if (weekday >= 0) return nextWeekday(now, weekday);
...
export function isDueThisMonth(due: string): boolean {
  const d = new Date(due);
  const now = new Date();
  return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
}

export function isDueThisQuarter(due: string): boolean {
  const d = new Date(due);
  const now = new Date();
  return d.getFullYear() === now.getFullYear() &&
...
export function isDueThisYear(due: string): boolean {
  const d = new Date(due);
...
function parseRelative(s: string): Date | null {
  ...
  if (unit.startsWith("d")) return addDays(now, n);
  if (unit.startsWith("w")) return addDays(now, n * 7);
