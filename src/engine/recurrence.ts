...
export function generateInstances(
  ...
  for (const template of templates) {
    ...
    if (pendingChildren.length >= limit) continue;
...
function addRecurrence(date: Date, recur: string): Date {
  ...
  if (!match) return new Date(date.getTime() + DAY_MS); // fallback: daily
  ...
  switch (unit) {
    ...
    case "monthly": { result.setMonth(result.getMonth() + 1); return result; }
    case "quarterly": { result.setMonth(result.getMonth() + 3); return result; }
    case "semiannual": { result.setMonth(result.getMonth() + 6); return result; }
    case "annual":
    case "yearly": { result.setFullYear(result.getFullYear() + 1); return result; }
    case "biannual":
    case "biyearly": { result.setFullYear(result.getFullYear() + 2); return result; }
  }

  // Numeric patterns
  if (unit.startsWith("d")) return new Date(date.getTime() + n * DAY_MS);
  if (unit.startsWith("w")) return new Date(date.getTime() + n * 7 * DAY_MS);
  if (unit === "m" || unit.startsWith("mo")) { result.setMonth(result.getMonth() + n); return result; }
  if (unit.startsWith("q")) { result.setMonth(result.getMonth() + n * 3); return result; }
  if (unit.startsWith("y")) { result.setFullYear(result.getFullYear() + n); return result; }
